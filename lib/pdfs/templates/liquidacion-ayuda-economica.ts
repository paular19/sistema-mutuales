...
import { DatosDocumento } from "@/lib/utils/documento-credito-pdflib";
...
/* ----------------- datos extendidos (sin tocar tu type) ----------------- */

type DeduccionItem = { label: string; tasaPct?: number; importe?: number };

function getExtras(datos: DatosDocumento) {
  const anyD = datos as any;

  // Estos pueden venir en cualquier lado según tu pipeline: arriba lo dejo centralizado
  ...
  const tasaExtraLabel =
    anyD?.liquidacion?.tasaExtraLabel ?? anyD?.tasaExtraLabel ?? (datos.credito as any)?.tasaExtraLabel ?? "CFT";

  const tasaExtraPct =
    anyD?.liquidacion?.tasaExtraPct ?? anyD?.tasaExtraPct ?? (datos.credito as any)?.tasaExtraPct;

  const deducciones: DeduccionItem[] =
    anyD?.liquidacion?.deducciones ??
    anyD?.deducciones ??
    (datos.credito as any)?.deducciones ??
    [];

  const totalDeducciones =
    anyD?.liquidacion?.totalDeducciones ??
    anyD?.totalDeducciones ??
    (datos.credito as any)?.totalDeducciones ??
    (deducciones.length ? deducciones.reduce((a, i) => a + (Number(i.importe) || 0), 0) : undefined);

  const importeNetoPercibir =
    anyD?.liquidacion?.importeNetoPercibir ??
    anyD?.importeNetoPercibir ??
    (datos.credito as any)?.importeNetoPercibir ??
    (typeof totalDeducciones === "number" ? datos.credito.monto - totalDeducciones : undefined);

  const cuotaMensual =
    anyD?.liquidacion?.cuotaMensual ??
    anyD?.cuotaMensual ??
    (datos.credito as any)?.cuotaMensual ??
...
function drawSignatureBlock(page: any, height: number, x: number, yTop: number, w: number, font: any) {
  ...
  page.drawText("Firma del Asociado", {
    x,
    y: yFromTop(height, lineY1),
    size: fs,
    font,
  });
  page.drawLine({
    start: { x, y: yFromTop(height, lineY1 + 12) },
    end: { x: x + w * 0.48, y: yFromTop(height, lineY1 + 12) },
    thickness: 1,
    color: rgb(0, 0, 0),
  });

  page.drawText("Sello y firma de la Mutual", {
    x: x + w * 0.52,
    y: yFromTop(height, lineY1),
    size: fs,
    font,
  ...
  page.drawLine({
    start: { x: x + w * 0.52, y: yFromTop(height, lineY1 + 12) },
    end: { x: x + w, y: yFromTop(height, lineY1 + 12) },
    thickness: 1,
    color: rgb(0, 0, 0),
  });
...
function drawCopy(
  ...
  const cuentaNumero = safe(a.codigo_externo, safe(a.convenio_numero, "")); // ajustá si vos sabés cuál es “Cta Nº”
  ...
  drawFit(
    "Liquidación de Ayuda Económica - Emisión de Ayudas Económicas Amortizables",
    marginX,
    topY + 18,
    11,
    blockW,
